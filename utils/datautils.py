...
def Load_liquidsugar_data(configs, scale = True):
    ...
    """
    # Forecasting task
    Data: original data (timestape, each stage variables, target variables)
    stage_vars: variables that constitue each stage
    segment_index: index points segmented according to seq_len + pred_len (consecutive time points)
    slices (train, val, test): # of train, val, test
    scalers: pretrained scalers using train data (standard scaler)
    """
    
    # load data
    data_path = 'Dataset/{}.csv'.format(configs.dataset) 
    ...
    # variable sort (timestape, stage1, stage2, stage3, target)
    data = data[['TimeStamp'] + x_vars + y_var]

    # Segmented index, which is actually used according to seq_len + pred_len
    segment_index = get_segment_index(data, configs)

    # Get splitted data point
